...
{
    $updateref = $db->query(
        "SELECT * FROM spectradb WHERE username = '$refer'"
    );
    ...
    if ($updateref) {
        if ($updateref->num_rows > 0) {
            ...
            $update_bal = $db->query(
                "UPDATE spectradb SET ref_bal = '$bal' WHERE email='$bal_email'"
            );
            if (!$update_bal) {
...
if (isset($_POST["register"])) {
    ...
    if ($password == $confirm_password) {
        $query = $db->query(
            "SELECT * FROM spectradb WHERE email='$email' or username='$username' #or phone='$phone'"
        );
        ...
        if ($query->num_rows > 0) {
            ...
            $createquery = $db->query(
                "INSERT INTO spectradb (name, email, username, password, phone, verify_token, referrer, created) VALUES('$name', '$email','$username', '$password','$phone', '$verify_token', '$refer', '$created')"
            );
