...
if (isset($_POST["upload_image"])) {
    ...
    $adduserimage = $db->query(
        "UPDATE spectradb SET user_image = '$filename' WHERE id = '$user_id' "
    );
...
if (isset($_POST["update_user_profile"])) {
    ...
    $checkquery = $db->query(
        "SELECT * FROM spectradb WHERE phone='$phone' or username='$username'"
    );
    ...
    if ($checkquery->num_rows > 1) {
        ...
        $update_profile = $db->query(
            "UPDATE spectradb SET user_image = '$update_filename', name = '$name', phone = '$phone', username = '$username' WHERE id = '$user_id' "
        );
