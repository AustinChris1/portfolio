...
if (isset($_POST["course_delete"])) {
    ...
    $coursedelete = $db->query(
        "DELETE FROM courses WHERE id = '$course_id' LIMIT 1"
    );
...
if (isset($_POST["course_update"])) {
    ...
    $courseupdate = $db->query(
        "UPDATE courses SET academy_id = '$academy_id', name = '$name', slug = '$slug', description = '$description', image = '$update_filename', status = '$status' WHERE id = '$course_id'"
    );
    ...
    if ($courseupdate) {
        if ($image != null) {
            ...
            move_uploaded_file(
                ...
                "../uploads/courses/" . $update_filename
            );
        }
...
if (isset($_POST["academy_update"])) {
    ...
    $academyupdate = $db->query(
        "UPDATE academy SET name='$name', slug='$slug', description= '$description', status='$status' WHERE id = '$academy_id'"
    );
    $academyupdate = $db->query(
        "ALTER TABLE spectradb  CHANGE COLUMN `$name` VARCHAR(255) NOT NULL"
    );
...
if (isset($_POST["academy_add"])) {
    ...
    $academyquery = $db->query(
        "INSERT INTO academy (name, slug, description, status) VALUES('$name', '$slug', '$description','$status')"
    );
    $academyquery = $db->query(
        "ALTER TABLE spectradb ADD COLUMN `$name` VARCHAR(255) NOT NULL AFTER status"
    );
    if ($academyquery) {
...
if (isset($_POST["post_update"])) {
    ...
    $postupdate = $db->query(
        "UPDATE posts SET category_id = '$category_id', name = '$name', slug = '$slug', description = '$description', image = '$update_filename', meta_title = '$meta_title', meta_description = '$meta_description', meta_keyword = '$meta_keyword', status = '$status' WHERE id = '$post_id'"
    );
...
if (isset($_POST["category_update"])) {
    ...
    $categoryupdate = $db->query(
        "UPDATE categories SET name='$name', slug='$slug', description='$description', meta_title='$meta_title', meta_description='$meta_description', meta_keyword='$meta_keyword', navbar_status='$navbar_status',status='$status' WHERE id = '$category_id'"
    );
...
if (isset($_POST["category_add"])) {
    ...
    $categoryquery = $db->query(
        "INSERT INTO categories (name, slug, description, meta_title, meta_description, meta_keyword, navbar_status, status) VALUES('$name', '$slug', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$navbar_status', '$status')"
    );
...
if (isset($_POST["adduser"])) {
    ...
    $checkquery = $db->query(
        "SELECT * FROM spectradb WHERE email='$email' or username='$username'"
    );
    ...
    if ($checkquery->num_rows > 0) {
        ...
        //inserting data
        $addquery = $db->query(
            "INSERT INTO spectradb (name, username, email, usertype, password, phone, status, created) VALUES('$name', '$username', '$email', '$role', '$password', '$phone', '$status','$created')"
        );
