    ...
    ?>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    if(isset($_GET['id'])){
                                        $course_id = $_GET['id'];
                                        $coursequery = $db->query("SELECT * FROM courses WHERE id='$course_id' LIMIT 1");
                                    ...
                                    <div class="row">
                                        ...
                                        <select name="academy_id" required class="form-control">
                                            ...
                                            foreach($academy as $academy_item){
                                                ...
                                                ?>
                                                <option value="<?=$academy_item['id']?>"<?=$academy_item['id'] == $courserow['academy_id'] ? 'selected' : '' ?>>
                                                <?=$academy_item['name']?>
                                            </option>
                                                <?php
                                            }
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <input type="hidden" name="course_id" value="<?=$courserow['id']?>">
                                            ...
                                            <input type="text" name="name" value="<?=$courserow['name']?>" required class="form-control">
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="">Slug(URL)</label>
                                            <input type="text" name="slug" value="<?=$courserow['slug']?>" required class="form-control">
                                        </div>
                                        <div class="col-md-12 mb-3">
                                            <label for="">Description</label>
                                            <textarea name="description" required class="form-control" id="summernote"  rows="4"><?= htmlentities($courserow['description'])?></textarea>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="">Image</label>
                                            <input type="hidden" name="old_image" value="<?=$courserow['image']?>" class="form-control"></textarea>
                                            <input type="file" name="image" class="form-control"></textarea>
                                        </div>

                                        <div class="col-md-6 mb-3">
                                        <label for="">Status</label><br>
                                            <input type="checkbox" name="status" <?=$courserow['status']=='1'?'checked':'';?> width="70px" height="70px"/> 
